...
export interface IAPICommunitiesHelpReturn {
  ok: boolean;
}

const CommunityPostHelped: NextApiHandler = async (req, res) => {
  const {
    query: { id: postId },
    session: { user },
    body: { answerId }
  ...
  const foundHelped = await client.help.findFirst({
    where: {
